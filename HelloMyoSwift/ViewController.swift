...
class ViewController: UIViewController, AVAudioRecorderDelegate, UITextFieldDelegate {
    ...
    let mediaCont = MPMusicPlayerController.systemMusicPlayer()
    var avAud = AVAudioPlayer()
    var count = Int()
    ...
    override func viewDidLoad() {
        ...
        TLMHub.shared().lockingPolicy = TLMLockingPolicy.none
        ...
        self.iniPitch = 0
        self.count = 0
        TLMHub.shared().lockingPolicy = TLMLockingPolicy(rawValue: 1)!
    ...
    func didSyncArm(_ notification: Notification) {
        ...
        armEvent.myo.vibrate(with: .medium)
    }
        ...
        case .waveIn:
            if host == 0 {
                ...
                if(self.pitch>CGFloat(lthresh) && self.pitch<CGFloat(mthresh)){
                    ...
                    lastPose.image = #imageLiteral(resourceName: "wisemiDark.png")
                    lastpitchL.text = "M"
                    if lPose1 != nil && lPose1.type == .waveOut && (lPose2 == nil || lPose2.type != .waveIn) {
                        mediaCont.skipToPreviousItem()
                    }
                }
        ...
        case .waveOut:
            if host == 0 {
                ...
                if(self.pitch>CGFloat(lthresh) && self.pitch<CGFloat(mthresh)){
                    ...
                    lastPose.image = #imageLiteral(resourceName: "wosemiDark.png")
                    lastpitchL.text = "M"
                    if lPose1 != nil && lPose1.type == .waveIn && (lPose2 == nil || lPose2.type != .waveOut) {
                        mediaCont.skipToNextItem()
                    }
                }
                if(self.pitch>CGFloat(mthresh)){
        ...
        case .fingersSpread:
            if host == 0 {
                ...
                if(self.pitch>CGFloat(lthresh) && self.pitch<CGFloat(mthresh)){
                    ...
                    lastPose.image = #imageLiteral(resourceName: "fssemiDark.png")
                    lastpitchL.text = "M"
                    if lPose1 == nil || lPose1.type != pose.type {
                        self.count+=1
                    }
                    if lPose1 != nil && lPose1.type == pose.type {
                        ...
                        if count % 2 == 0 {
                            if mediaCont.playbackState == .paused || mediaCont.playbackState == .stopped {
                                mediaCont.play()
                            } else {
                                mediaCont.pause()
                            }
...
//            }
        ...
        if (currentPose.type == .doubleTap || currentPose.type == .fist || currentPose.type == .fingersSpread || currentPose.type == .waveIn || currentPose.type == .waveOut) {
            if lGes != nil && lGes.type != pose.type {
                lGes = pose
            }
            lPose6 = lPose5
            lPose5 = lPose4
